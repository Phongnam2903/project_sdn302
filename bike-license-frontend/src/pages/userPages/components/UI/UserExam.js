...
import { Button, Card, Row, Col, Form, Toast, ToastContainer, Modal, ProgressBar, Badge, Alert } from "react-bootstrap"
...
function UserExam() {
    ...
    const fetchExam = async () => {
      ...
      if (!token) {
        ...
        const timer = setInterval(() => {
          ...
          const newProgress = Math.max(0, 100 - (elapsed / toastDuration) * 100)
      ...
      try {
        ...
        if (!res.data || !res.data.questions) {
          throw new Error("Không tìm thấy dữ liệu đề thi hoặc danh sách câu hỏi")
        }

        setExam(res.data)
      } catch (error) {
        console.error("Lỗi khi fetch đề thi:", error)
        alert("Lỗi tải đề thi: " + (error.response?.data?.error || error.message))
      }
    }
  ...
  const getScore = () => {
    ...
    return exam.questions.reduce((acc, q) => {
      ...
      return selected !== undefined && q.answers[selected]?.isCorrect ? acc + 1 : acc
    }, 0)
  }
      ...
      <Layout>
        <ToastContainer position="top-end" className="p-3" style={{ zIndex: 9999 }}>
          ...
          >
            ...
            <Toast.Body className="fw-semibold text-dark text-center">Vui lòng đăng nhập để làm bài thi.</Toast.Body>
          </Toast>
        </ToastContainer>
      </Layout>
    )
    ...
    <Layout>
      <div className="exam-container">
            ...
            <Row className="align-items-center">
              <Col md={3}>
                <div className="time-display">
                  <Clock className={`time-icon text-${getTimeColor()}`} size={20} />
                  <div className="time-info">
                    <div className={`time-value text-${getTimeColor()}`}>{formatTime(timeLeft)}</div>
                ...
                <div className="progress-section">
                  ...
                  <ProgressBar className="custom-progress">
                    ...
                    <ProgressBar
                      ...
                      style={{ position: "absolute", top: 0, height: "4px", opacity: 0.7 }}
                    />
                  </ProgressBar>
                </div>
              </Col>
              <Col md={3}>
                <div className="stats-display">
                  ...
                  <div className="stat-item">
                    <AlertTriangle className="stat-icon text-warning" size={16} />
                    <span>{exam.questions.length - getAnsweredCount()} còn lại</span>
                  </div>
                </div>
              </Col>
            </Row>
        ...
        {showTimeWarning && timeLeft <= 5 * 60 && timeLeft > 0 && (
          <Alert variant="warning" className="time-warning-alert" dismissible onClose={() => setShowTimeWarning(false)}>
            <AlertTriangle className="me-2" size={16} />
            <strong>Cảnh báo:</strong> Chỉ còn {Math.ceil(timeLeft / 60)} phút! Hãy hoàn thành bài thi.
          </Alert>
        )}

        {/* Result Display */}
        {finished && (
          <Alert variant={getScore() >= 21 ? "success" : "danger"} className="result-alert">
            <div className="result-content">
              ...
              <div className="result-info">
                <h5 className="mb-1">{getScore() >= 21 ? "🎉 Chúc mừng! Bạn đã đạt" : "😔 Rất tiếc! Bạn chưa đạt"}</h5>
                <p className="mb-0">
                  Điểm số:{" "}
                  <strong>
                    {getScore()}/{exam.questions.length}
                  </strong>{" "}
                  - <strong>{Math.round((getScore() / exam.questions.length) * 100)}%</strong>
                </p>
              </div>
              <Button variant="primary" onClick={() => navigate("/profile")} className="result-button">
                ...
                <div className="question-grid">
                  {exam.questions.map((q, idx) => {
                    ...
                    return (
                      ...
                      >
                        {idx + 1}
                        {isAnswered && <CheckCircle className="check-icon" size={12} />}
                      </Button>
                    )
                  })}
                </div>
            ...
            <Card className="question-card">
              <Card.Header className="question-header">
                <div className="d-flex justify-content-between align-items-center">
                  ...
                  <div className="question-timer">
                    <Clock className={`me-1 text-${getTimeColor()}`} size={16} />
                    <span className={`fw-bold text-${getTimeColor()}`}>{formatTime(timeLeft)}</span>
                  </div>
                </div>
              </Card.Header>
              ...
              <Card.Body className="question-body">
                <div className="question-content">
                  <h5 className="question-text">{exam.questions[current].content}</h5>
                    ...
                    {exam.questions[current].answers.map((a, idx) => (
                      <div key={idx} className="answer-option">
                        <Form.Check
                          ...
                          disabled={finished}
                          onChange={() => handleSelect(exam.questions[current]._id, idx)}
                ...
                <div className="question-actions">
                  <div className="navigation-buttons">
                    ...
                    <Button
                      ...
                      disabled={current === exam.questions.length - 1 || finished}
                      className="nav-button"
                    >
                      Câu tiếp
        ...
        <Modal show={showSubmitModal} onHide={() => setShowSubmitModal(false)} centered className="submit-modal">
          ...
          <Modal.Body className="text-center py-4">
            <div className="submit-summary">
              ...
              <div className="summary-item">
                <AlertTriangle className="summary-icon text-warning" size={20} />
                <span>
                  Chưa làm: <strong>{exam.questions.length - getAnsweredCount()}</strong> câu
                </span>
              </div>
              <div className="summary-item">
            ...
            <p className="mt-3 text-muted">Bạn có chắc chắn muốn nộp bài? Hành động này không thể hoàn tác.</p>
